...
The method is generic on the `Success` type of the completion's `Result` parameter. This means that callers must ensure their completions are explicit, which isn't always a nice thing to demand inside of, say, a view controller. To ease this, I like to define a static function for each of my endpoints:
